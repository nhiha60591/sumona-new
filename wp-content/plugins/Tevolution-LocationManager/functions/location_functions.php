...
/*
 * directory_posts_custom_column function use for returl city name from city id
 */
...
function tmpl_locations_color_settings(){
	...
	if(($current_cityinfo['color'] && $current_cityinfo['color'] !='#') || $current_cityinfo['images'] || ($current_cityinfo['header_color'] && $current_cityinfo['header_color'] !='#') || $current_cityinfo['header_image']):?>
		<style type="text/css">
			html body, #main{ <?php if($current_cityinfo['color']):?> background-color:<?php echo $current_cityinfo['color'];?>; <?php endif;?> <?php if($current_cityinfo['images']):?> background-image:url('<?php echo $current_cityinfo['images'];?>'); <?php endif;?> }
