...
function location_selection_option(){
	...
	?>
     <tr class="directory_city_option">
		...
		<p class="description"><?php echo __('City selector is used to change the current city. It is located inside the header. ',LMADMINDOMAIN);?></p>	</td>		
     </tr>
	 <tr class="directory_city_option">
				...
				<tr class="directory_city">
					...
					<td><?php $cityinfo = $wpdb->get_results("SELECT cityname FROM $multicity_table where is_default=1");?>                   
						...
						<p class="description" id="default_city_set_msg" <?php if($default_city_set!='default_city'){ echo "style='display:none'"; } ?>><?php echo __('Displays listings from your default city to visitors once they land on your homepage.',LMADMINDOMAIN);?></p>
                              <p class="description" id="nearest_city_set_msg" <?php if($default_city_set!='nearest_city'){ echo "style='display:none'"; } ?>><?php echo __('Show listings from the city nearest to the visitors without asking to share their location.',LMADMINDOMAIN);?></p>
...
class wp_list_manage_multicitiy extends WP_List_Table 
{
	...
	{	
		...
		$actions = array(
				...
				'edit' => sprintf('<a href="?page=%s&action=%s&cf=%s&%s">'.__('Edit',LMADMINDOMAIN).'</a>',$_REQUEST['page'],'edit',$item['ID'],'location_tabs=city_manage_locations'),
				'delete' => $delete_url,
...
function location_fill_states_cmb()
{
	...
	if($zonesinfo):		
		foreach($zonesinfo as $zone):
			...
			$zones_ontion.='<option value="'.$zone->zones_id.'"  '.$selected.'>'.htmlspecialchars($zone_name).'</option>';
		endforeach;
	else:
...
function location_current_multicity(){
	...
	if(isset($_REQUEST['find_city']) && $_REQUEST['find_city']=='nearest'){
		...
		$sql="SELECT distinct city_id, cityname,city_slug FROM  $multicity_table, {$wpdb->prefix}postmeta WHERE meta_key='post_city_id' AND meta_value=city_id ORDER BY truncate((degrees(acos( sin(radians(lat)) * sin( radians('".$lat."')) + cos(radians(lat)) * cos( radians('".$lat."')) * cos( radians(lng - '".$long."') ) ) ) * 69.09),1) ASC LIMIT 0,1";
	...
	$cityinfo = $wpdb->get_results($wpdb->prepare("SELECT mc.*,mc.message as msg,c.country_name,c.country_flg,z.zone_name FROM $multicity_table mc,$zones_table z,$country_table c where c.country_id=mc.country_id AND z.zones_id=mc.zones_id AND  mc.city_id =%d order by cityname   ASC",$_SESSION['post_city_id']));
