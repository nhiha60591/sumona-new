...
/*
	Display the Screen name on user profile fields section
 */
						...
						<tr>
							...
							<td width="90%" style="line-height:10px">
								...
								$settings =   array(
												'wpautop' => false, // use wpautop?
												'media_buttons' => false, // show insert/upload button(s)
												'textarea_name' => 'admin_registration_success_email_content', // set the textarea name to something different, square brackets [] can be used here
												'textarea_rows' => '7', // rows="..."
												'tabindex' => '',
												'editor_css' => '<style>.wp-editor-wrap{width:640px;margin-left:0px;}</style>', // intended for extra styles for both visual and HTML editors buttons, needs to include the <style> tags, can use "scoped".
												'editor_class' => '', // add extra class(es) to the editor textarea
...
function templatic_general_setting_register_data($column){
	...
	<?php
	/*
	 * function to hide show the html of social loggin.
	 */	
...
function show_social_login()
{
	?>
		<script>
			function show_social_login(val)
			{
				if(val == 'allow_facebook_login')
				{
					jQuery('#show_facebook_key').toggle();
				}
				else if(val == 'allow_google_login')
				{
					jQuery('#show_google_key').toggle();
				}
				else if(val == 'allow_twitter_login')
				{
					jQuery('#show_twitter_key').toggle();
				}
			}
		</script>
	<?php
