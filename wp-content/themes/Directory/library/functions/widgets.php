	...
	*/
	function widget( $sidebar, $instance )
	{
		...
		if ( $d ) {
			...
			$args1['show_option_none'] = __('Select Category',THEME_DOMAIN);
			...
			wp_dropdown_categories( apply_filters( 'widget_categories_dropdown_args', $args1 ) );
			...
			jQuery(document).ready(function(){
				...
				jQuery('#dropcat').live('change',function(){
						...
						if('<?php echo $args['taxonomy'];?>' == 'category'){
							...
							cat_name = jQuery("#dropcat option[value="+cat_id+"]").text().replace(/\s/g,"-");
...
if(!class_exists('supreme_subscriber_widget'))
{
	class supreme_subscriber_widget extends WP_Widget
	{
		...
		if($newsletter_provider == 'feedburner')
		{
			...
			<div class="subscriber_container">
				<form action="http://feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open('http://feedburner.google.com/fb/a/mailverify?uri=<?php echo $feedburner_id; ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520');return true" >
					<input type="text" id="newsletter_name" name="name" value="" class="field" onfocus="if (this.placeholder == '<?php _e('Your Name',THEME_DOMAIN); ?>') {this.placeholder = '';}" onblur="if (this.placeholder == '') {this.placeholder = 'Your Name';}" placeholder="<?php _e('Your Name',THEME_DOMAIN); ?>" />
		...
		{	
			...
			?>
		  <div class="subscriber_container">
			<input type="text" name="name" id="name" value="" placeholder="<?php _e('Name',THEME_DOMAIN);?>" class="field" onfocus="if (this.placeholder == 'Name') {this.placeholder = '';}" onblur="if (this.placeholder == '') {this.placeholder = 'Name';}"  />
	...
	class supreme_banner_slider extends WP_Widget
	{
		function __construct()
		{
			parent::__construct(
				...
				array('description'=> apply_filters('tmpl_banner_widget_description',__('Display images from a specific category or a collection of static images. The widget works best in the Homepage Slider area.',ADMINDOMAIN)))
			);
		}
			...
			*/
			...
			else
			{
				if($is_Carousel)
				{
					...
					$post_image = get_the_image( array('size' => $slider_image_size,'echo'=> false ,'width'        => $width,'height'       => $height,'default_image'=> "http://placehold.it/60x60"));
					$flag = 1;
				}
	...
	class supreme_contact_widget extends WP_Widget
	{
		...
		function form($instance)
		{
			...
			<input type="checkbox" value="1" <?php echo ($contactus_captcha==1) ? 'checked' : ''?>  id="<?php echo $this->get_field_id( 'contactus_captcha' ); ?>" name="<?php echo $this->get_field_name('contactus_captcha'); ?>">
                ...
                <?php echo __('Enable captcha (Wp-reCaptcha plugin should be activate)',ADMINDOMAIN);?>
              </label>
             </p>
			<?php
		}
